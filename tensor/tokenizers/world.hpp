...
// ostream_iterator
#include <iterator>
#include "tokenizers/world_list.hpp"
...
public:
    RWKVTokenizer() {
        ...
        std::string tokens = WorldTokens;
        // split file into lines
        std::istringstream f(tokens);
        std::string line;
        while (std::getline(f, line)) {
            filelines.push_back(line);
            sorted.push_back(std::vector<uchar>());
        }
        ...
        std::chrono::duration<double> elapsedprocess = afterprocess - beforeprocess;
        std::cout << "Time taken to process file: " << elapsedprocess.count() << " s" << std::endl;
        std::cout << std::endl;
...
RWKVTokenizer world = RWKVTokenizer();
// size_t main() {
